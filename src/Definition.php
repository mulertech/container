...
class Definition
{
    ...
    public function getControllerFunction(string $function, Container $container): mixed
    {
        ...
        if (!method_exists($controllerInstance, $function)) {
            throw new NotFoundException(sprintf('The method "%s" of the controller "%s" doesn\'t exist.', $function, $controllerInstance::class));
        }
    ...
    private function resolveArgument(Container $container, \ReflectionParameter $arg): mixed
    {
        ...
        if (null === $this->getArgumentClass($arg)) {
            if ($arg->isDefaultValueAvailable()) {
                return $arg->getDefaultValue();
            }

            throw new NotFoundException(sprintf('The class or interface : %s has a missing argument named : %s.', $this->id, $arg->getName()));
        }

        // If argument is a container interface give it the $container
        if (ContainerInterface::class === $this->getArgumentClass($arg)->getName()) {
            return $container;
        }
    ...
    private function getReflectionClass(): \ReflectionClass
    {
        if (!class_exists($this->id) && !interface_exists($this->id)) {
            throw new NotFoundException(sprintf('The class or interface : %s doesn\'t exist or is not accessible.', $this->id));
        }
        ...
        if ($reflexionClass->isInterface() || $reflexionClass->isAbstract()) {
            if (empty($this->alias)) {
                throw new NotFoundException(sprintf('The alias of this interface or abstract class was not found : %s', $this->id));
            }
